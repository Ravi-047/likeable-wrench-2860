...
import {Box,SimpleGrid,Image,Button,Heading,Text, Flex,Divider, Icon, Slider, Input, Checkbox, SliderMark, SliderTrack, SliderFilledTrack, SliderThumb} from "@chakra-ui/react"
...
function Product(){
   ...
   const getProd=()=>{
    axios.get(`http://localhost:8080/furniture?category=${checkboxvalue}`)
        ...
        <Flex pr="8%" pl="8%" gap="3%">
            <Box w="30%"  h="200px"  >
               <Flex mt="1rem" mb="1rem"  justifyContent="space-between">
                  ...
                  <Button border="1px solid #e6e6e6" bg="none">Reset</Button>
               </Flex>
        ...
        >
          {sliderValue}
               ...
               <Box border="1px solid #e6e6e6" p="8%" mt="2rem">
                <Text fontSize="lg" mb="1rem">PRODUCT TYPE</Text>
                {catData.map((ele)=>(
                   <Flex mt="1rem">
                   <Checkbox onChange={checkboxhandler} value={ele.name}>{ele.name}</Checkbox>
               </Flex>
                ))}
               ...
               <Box p="8%" border="1px solid #e6e6e6" mt="2rem">
                <Text fontSize="lg">AVAILABILITY</Text>
                 <Checkbox  mt="1rem" defaultChecked value="Out of Stock">Out of Stock</Checkbox>
            ...
            <Box w="100%" >
                ...
                <Flex  >
                    <Text fontSize="lg" mr="0.5rem">{para}</Text>
                    <Link to={`/${param}`} ><AiOutlineClose size="25px"/></Link>
                </Flex>
                </Box>
                <SimpleGrid columns={[2,2,3]} columnGap="5%"rowGap="1%">
                  {proddata.map((prod)=>(
                    ...
                    <Box bg="white" border="1px solid #e6e6e6">
                        ...
                        <Divider mt="3%" mb="3%" orientation='horizontal' bg="red"/>
